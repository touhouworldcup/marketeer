...
public:
    ...
    void* ExternalLaunch(const char* game, std::string& ctx, int type)
    {
        auto gameIt = mGames.find(game);
        if (gameIt == mGames.end()) {
            return false;
        }

        auto& instances = gameIt.value().instances;
        for (size_t i = 0; i < instances.size(); ++i) {
            if (instances[i].type == type) {
                return (void*)LaunchGameDirectly(&gameIt.value(), i, ctx);
            }
        }
        return 0;
    }

    static bool LocalApplyTHPrac(HANDLE process, std::string& mktCtx)
    {
        void* extraData = nullptr;
        size_t extraSize = 0;
        if (mktCtx != "") {
            extraData = (void*)mktCtx.c_str();
            extraSize = mktCtx.size();
        }
    ...
    static DWORD WINAPI WaitAndApplyTHPrac(std::wstring& exePath, std::string& mktCtx)
    {
        auto& gameGui = THGameGui::singleton();
...
#undef MakePointer
    ...
    static HANDLE WINAPI LaunchGameDirectly(THGame* currentGame, int instance, std::string& ctx)
    {
        bool result = true;
        //auto currentGame = THGameGui::singleton().mCurrentGame;
        if (!currentGame) {
        ...
        if (currentInst.useTHPrac || ctx != "") {
            result = (WriteTHPracSig(proc_info.hProcess) && LocalApplyTHPrac(proc_info.hProcess, ctx));
        }
        ...
        if (!result) {
            ...
            if (ctx.size() >= 4) {
                auto magic = *(uint32_t*)ctx.data();
                if (magic == 'MRTS') {
                    LauncherMktLaunchCallback(currentGame->signature.idStr);
                }
            }
    ...
    static DWORD WINAPI LaunchThreadFunc(_In_ LPVOID lpParameter)
    {
        ...
        case TYPE_TCHINESE:
            if (currentCatagory == CAT_MAIN || currentCatagory == CAT_SPINOFF_STG) {
                if (useReflectiveLaunch) {
                    ...
                    auto handle = LaunchGameDirectly(THGameGui::singleton().mCurrentGame, THGameGui::singleton().mCurrentGame->selected, mktCtx);
                    if (handle) {
                        CloseHandle(handle);
                        return 1;
                    }
        ...
        if ((currentCatagory == CAT_MAIN || currentCatagory == CAT_SPINOFF_STG) && (currentInst.useTHPrac || mktCtx != "")) {
            return WaitAndApplyTHPrac(currentInstExePath, mktCtx);
        }

        if (mktCtx.size() >= 4) {
            auto magic = *(uint32_t*)mktCtx.data();
            if (magic == 'MRTS') {
                LauncherMktLaunchCallback(currentGame->signature.idStr);
            }
        }
    ...
    void GameVersionTable()
    {
        ...
        if (mktCtx.size()) {
            isStreaming = true;
            auto mutex = OpenMutexA(SYNCHRONIZE, 0, mktCtx.c_str());
            if (mutex) {
                mutexStatus = true;
                CloseHandle(mutex);
            } 
        ...
        auto needDisabled = (isStreaming && currentInst.type != TYPE_ORIGINAL) || mutexStatus;
        if (needDisabled) {
            ImGui::BeginDisabled();
        }
...
void* LauncherGamesExternalLaunch(const char* game, std::string ctx, int type)
{
    return THGameGui::singleton().ExternalLaunch(game, ctx, type);
}
}